package com.aczuleta.teams.prediction.system.routing

import com.aczuleta.teams.prediction.system.serializers.JsonSupport
...
trait MyResource extends Directives with JsonSupport {
  ...
  def complete(resource: Future[Unit]): Route =
    onSuccess(resource) { complete(204, None) }
